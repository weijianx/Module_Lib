...
float gTemBuf[1][10];
float gHumBuf[1][10];
SensorSta_Typedef SensorSta[2*1] = {NExist};
SHT3_Typedef T_H;
...
void Get_Duration(void)
{
    static uint32_t TemCountCnt1 = 0,TemCountCnt2 = 0;
    uint8_t i,k;

    for(i = 0; i < 1; i++)
    {
        if(T_H.Temp[i] > T_H.UpThreshold[i])
        {
            if(!(T_H.AlarmSta[i] & 0x00010000))
            {
                if(TemCountCnt1++ >= T_H.DurThreshold[i] - 1)
                {
                    TemCountCnt1 = 0;
        ...
        else if(T_H.Temp[i] < T_H.DoThreshold[i])
        {
            if(!(T_H.AlarmSta[i] & 0x00000001))
            {
                if(TemCountCnt1++ >= T_H.DurThreshold[i] - 1)
                {
                    TemCountCnt1 = 0;
    ...
    {
       k = i - 1;
      if(T_H.Humi[k] > T_H.UpThreshold[i])
        {
            if(!(T_H.AlarmSta[i] & 0x00010000))
            {
                if(TemCountCnt2++ >= T_H.DurThreshold[i] - 1)
                {
                    TemCountCnt2 = 0;
                    T_H.AlarmSta[i] |= 0x00010000;
                    T_H.AlarmSta[i] &= 0x00010000;
        ...
        else if(T_H.Humi[k] < T_H.DoThreshold[i])
        {
            if(!(T_H.AlarmSta[i] & 0x00000001))
            {
                if(TemCountCnt2++ >= T_H.DurThreshold[i] - 1)
...
void SHT3_Init(void)
{
    ...
    for(i = 0; i < 1; i++)
    {
        sta = Set_Sht3_Periodi(i, 0x2329);
        if(sta)
        {
            SensorSta[i + 1] = SensorSta[i] = NExist;
        }
        else
        {
            SensorSta[i + 1] = SensorSta[i] = Exist;
        }
    }
}
# 190 "../BSP/Src/SHT3x.c"
uint8_t SHT3x_WriteData(uint8_t DriverAddr, uint16_t Command)
{
    ...
    TH_IIC_Start();
    TH_IIC_Send_Byte(DriverAddr);
    sta += TH_IIC_Wait_Ack();
    TH_IIC_Send_Byte((Command >> 8) & 0xff);
    sta += TH_IIC_Wait_Ack();
    TH_IIC_Send_Byte(Command & 0xff);
    sta += TH_IIC_Wait_Ack();
    TH_IIC_Stop();
...
uint8_t SHT3x_ReadData(uint8_t DriverAddr, uint16_t Command, uint8_t* DataBuff, uint8_t DataLen)
{
    ...
    TH_IIC_Start();
    TH_IIC_Send_Byte(DriverAddr + 1);
    sta += TH_IIC_Wait_Ack();

    for(i = 0; i < DataLen - 1; i++)
    {
        *(DataBuff + i) = TH_IIC_Read_Byte(1);
    }
    *(DataBuff + i) = TH_IIC_Read_Byte(0);
    TH_IIC_Stop();
...
uint8_t Set_Sht3_Periodi(uint8_t sChannal, uint16_t sPeriodi)
{
    ...
    sStatus = SHT3x_WriteData(0x88, 0x2329);
    return sStatus;
}
...
uint8_t Read_Sht3_TH(uint8_t sChannal, uint16_t sReadCmd, uint8_t *sReadBuf, uint16_t sReadLen)
{
    uint8_t sStatus;
    Get_Channal_Pin(sChannal);
    sStatus = SHT3x_ReadData(0x88, sReadCmd, sReadBuf, sReadLen);
    return sStatus;
}
# 286 "../BSP/Src/SHT3x.c"
float TH_Filter(float NewData, float* FilterBuf, uint16_t FilterNum, uint16_t DelHeadNum, uint16_t DelTailNum)
{
    ...
    memcpy(FilterArray, FilterBuf, sizeof(FilterArray));

    for (i = 0; i < DelTailNum; i ++)
    {
    ...
    for(i = DelHeadNum; i < FilterNum - DelTailNum; i ++)
        filter_sum += FilterArray[i];

    filter_sum /= (FilterNum - (DelHeadNum + DelTailNum));

    memset(FilterArray, 0x00, sizeof(FilterArray));
...
void ReadTH(void)
{
 ...
 static uint8_t sing = 0;
    ...
    while(1)
    {
            ...
            for(i = 0; i < 1; i++)
            {
                status = Read_Sht3_TH(i, 0xE000, sDataBuff, sizeof(sDataBuff));

                if(status)
                {
                    SensorSta[i] = NExist;
                    SensorSta[i + 1] = SensorSta[i];
                }
                else
                {
                    SensorSta[i] = Exist;
