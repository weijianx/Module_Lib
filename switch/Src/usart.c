...
static BitAction UartRecvNewData = Bit_RESET;
...
{  
    ...
    case 02:
        MODIFY_REG(LPUART1->CR1, ((0x10001U << 12) | (1 << 10) | (1 << 9) | (1 << 3) | (1 << 2) | (1 << 15)), 
              UART_WORDLENGTH_9B | UART_PARITY_EVEN | UART_MODE_TX_RX);               //配置数据位，校验位，串口模式
        break;
    case 01:
        MODIFY_REG(LPUART1->CR1, ((0x10001U << 12) | (1 << 10) | (1 << 9) | (1 << 3) | (1 << 2) | (1 << 15)), 
              UART_WORDLENGTH_9B | UART_PARITY_ODD | UART_MODE_TX_RX);               //配置数据位，校验位，串口模式
        break;
    default:
        MODIFY_REG(LPUART1->CR1, ((0x10001U << 12) | (1 << 10) | (1 << 9) | (1 << 3) | (1 << 2) | (1 << 15)), 
              UART_WORDLENGTH_8B | UART_PARITY_NONE | UART_MODE_TX_RX);               //配置数据位，校验位，串口模式
...
//        MODBUS_ASCII_HandlRevData(RecvByteData); //接收ASCII数据     
        ...
        {
            UART1_RXBuff[UART1_RXBuffLen] = LPUART1->RDR;   
            UART1_RXBuffLen++;
...
//    ckSourceConfig.ClockSource = TIM_CLOCKSOURCE_INTERNAL;
...
void TIM2_IRQHandler(void)
{
    ...
    {
        __HAL_TIM_GET_IT_SOURCE(&htim2, TIM_FLAG_UPDATE);
